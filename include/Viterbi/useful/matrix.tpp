/**
 * @brief C++11 matrix implementation
 * @note (highly) inspired by https://gist.github.com/MihailJP/3937550
 * */

#ifndef USEFULL_MATRIX_TPP
#define USEFULL_MATRIX_TPP

#include "useful/matrix.hpp"  //Useless. Just for linter
...
void Matrix<T>::resize(size_t i, size_t j) {
  ...
  for (auto& col : data) {
    if (col.get() == nullptr) {
      col = std::make_shared<std::vector<T>>();
    }
    col->resize(j);
  }
}
...
template <typename T>
size_t Matrix<T>::nb_col() const {
  return nbCol;
}

template <typename T>
size_t Matrix<T>::nb_row() const {
  return nbRow;
}
...
template <typename T>
void Matrix<T>::set(const T& val, size_t i, size_t j) {
  (*data[i])[j] = val;
}
...
// /* Row getter */
// MTXTMP std::array<T, cols> matrix<T, rows, cols>::getrow(
//     unsigned int row) const {
//   return std::array<T, cols>(myVal[row]);
// }

// /* Column getter */
// MTXTMP std::array<T, rows> matrix<T, rows, cols>::getcol(
...
// /* Getter */
// MTXTMP T matrix<T, rows, cols>::getval(unsigned int row,
//                                        unsigned int col) const {
//   return myVal[row][col];
// }

// /* Setter */
// MTXTMP void matrix<T, rows, cols>::setval(unsigned int row,
//                                           unsigned int col,
//                                           const T val) {
//   myVal[row][col] = val;
//   return;
...
// /* Addition */
// MTXTMP matrix<T, rows, cols> matrix<T, rows, cols>::operator+(
//     const matrix<T, rows, cols>& operand) const {
...
// }
// MTXTMP matrix<T, rows, cols>& matrix<T, rows, cols>::operator+=(
//     const matrix<T, rows, cols>& operand) {
//   for (unsigned int i = 0; i < rows; i++)
//     for (unsigned int j = 0; j < cols; j++)
...
// MTXTMP template <unsigned int opcols>
// matrix<T, rows, opcols> matrix<T, rows, cols>::operator*(
...
//         ans.setval(
//             i, j, ans.getval(i, j) + this->getval(i, k) * operand.getval(k,
//             j));
//   return ans;
// }
...
// T_ matrix<T, rows, cols>::calc_det<T_, rows_, cols_>::operator()(
...
// MTXTMP template <typename T_>
// T_ matrix<T, rows, cols>::calc_det<T_, 1, 1>::operator()(
//     const matrix<T_, 1, 1>& arg) const {
//   return arg.getval(0, 0);
// }

// /* 2x2 matrix determinant */
// MTXTMP template <typename T_>
// T_ matrix<T, rows, cols>::calc_det<T_, 2, 2>::operator()(
//     const matrix<T_, 2, 2>& arg) const {
//   return arg.getval(0, 0) * arg.getval(1, 1) -
//          arg.getval(0, 1) * arg.getval(1, 0);
// }

// /* nxn matrix determinant */
// MTXTMP template <typename T_, unsigned int size>
// T_ matrix<T, rows, cols>::calc_det<T_, size, size>::operator()(
//     const matrix<T_, size, size>& arg) const {
//   T_ det;
...
// matrix<T, rows, cols>::calc_inv<T_, rows_, cols_>::operator()(
//     const matrix<T_, rows_, cols_>&) const {
//   throw std::domain_error("Not a square matrix");
// }
...
// MTXTMP template <typename T_>
// const matrix<T_, 1, 1> matrix<T, rows, cols>::calc_inv<T_, 1, 1>::operator()(
//     const matrix<T_, 1, 1>& arg) const {
...
// MTXTMP template <typename T_>
// const matrix<T_, 2, 2> matrix<T, rows, cols>::calc_inv<T_, 2, 2>::operator()(
//     const matrix<T_, 2, 2>& arg) const {
//   T_ det = arg.determinant();
//   if (det == T_(0))
//     throw std::domain_error("Cannot invert a singular matrix");
...
// matrix<T, rows, cols>::calc_inv<T_, size, size>::operator()(
//     const matrix<T_, size, size>& arg) const {
...
//           minor.setval(
//               p, q, arg.getval((p >= i) ? (p + 1) : p, (q >= j) ? (q + 1) :
//               q));
//       if ((i + j) % 2 == 0)
...
//     const matrix<T_, rows_, cols_>& operand) const {
...
// MTXTMP template <typename T_, unsigned int rows_, unsigned int cols_>
// bool matrix<T, rows, cols>::calc_uptriag<T_, rows_, cols_>::operator()(
//     const matrix<T_, rows_, cols_>&) const {
//   throw std::domain_error("Not a square matrix");
// }
...
// MTXTMP template <typename T_, unsigned int size>
// bool matrix<T, rows, cols>::calc_uptriag<T_, size, size>::operator()(
//     const matrix<T_, size, size>& arg) const {
//   for (int i = 0; i < size; i++)
//     for (int j = 0; j < i; j++)
...
// MTXTMP template <typename T_, unsigned int rows_, unsigned int cols_>
// bool matrix<T, rows, cols>::calc_lwtriag<T_, rows_, cols_>::operator()(
//     const matrix<T_, rows_, cols_>&) const {
//   throw std::domain_error("Not a square matrix");
// }
...
// MTXTMP template <typename T_, unsigned int size>
// bool matrix<T, rows, cols>::calc_lwtriag<T_, size, size>::operator()(
//     const matrix<T_, size, size>& arg) const {
//   for (int i = 0; i < size; i++)
