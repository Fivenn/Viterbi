...
// Unix
      ...
      ELPP_OS_SOLARIS || ELPP_OS_AIX || ELPP_OS_EMSCRIPTEN) &&             \
     (!ELPP_OS_WINDOWS))
...
enum class ConfigurationType : base::type::EnumType {
  ...
  Format = 8,
  /// @brief Determines log file (full path) to write logs to for correponding
  /// level and logger
  Filename = 16,
...
class ConfigurationTypeHelper : base::StaticClass {
 public:
  ...
  /// @brief Casts int(ushort) to configurationt type, useful for iterating
  /// through enum.
...
enum class LoggingFlag : base::type::EnumType {
  ...
  /// @brief Flushes log with every log-entry (performance sensative) - Disabled
  /// by default
  ImmediateFlush = 16,
...
const struct {
  ...
  const char* detail;
    ...
    {SIGFPE, "SIGFPE", "Erroneous arithmetic operation",
     "Arithemetic operation issue such as division by zero or operation "
...
class Str : base::StaticClass {
 public:
  ...
  static char* convertAndAddToBuff(std::size_t n,
                                   int len,
                                   char* buf,
                                   const char* bufLim,
...
class DateTime : base::StaticClass {
 public:
  ...
  /// Windows, a seperate implementation is provided
 ...
 private:
  static char* parseFormat(char* buf,
                           std::size_t bufSz,
                           const char* format,
...
class CommandLineArgs {
 public:
  ...
  /// @brief Returns true if arguments contain paramKey with a value (seperated
  /// by '=')
...
class Registry
 ...
 public:
  ...
  /// @brief Assignment operator that unregisters all the existing registeries
...
class RegistryWithPred : public AbstractRegistry<T_Ptr, std::vector<T_Ptr*>> {
 ...
 protected:
  ...
  /// @brief Gets pointer from repository with speicifed arguments. Arguments
...
class Configurations
 ...
 private:
  ...
  void unsafeSetIfNotExist(Level level,
                           ConfigurationType configurationType,
                           const std::string& value);

  /// @brief Thread unsafe set
  void unsafeSet(Level level,
                 ConfigurationType configurationType,
                 const std::string& value);
...
class RegisteredHitCounters
 ...
 public:
  ...
  bool validateEveryN(const char* filename,
                      base::type::LineNumber lineNumber,
                      std::size_t n);
  ...
  /// @return True if validation resulted in triggering hit. Meaning logs should
  /// be written everytime true is returned
  bool validateAfterN(const char* filename,
                      base::type::LineNumber lineNumber,
                      std::size_t n);
  ...
  /// @return True if validation resulted in triggering hit. Meaning logs should
  /// be written everytime true is returned
  bool validateNTimes(const char* filename,
                      base::type::LineNumber lineNumber,
                      std::size_t n);
...
class MessageBuilder {
 public:
  MessageBuilder(void)
      : m_logger(nullptr), m_containerLogSeperator(ELPP_LITERAL("")) {}
...
#define ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG(temp)                           \
  ...
  /// @param ElementInstance Instance of element to be fed out. Insance name is
...
#undef ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG
  ...
  MessageBuilder& writeIterator(Iterator begin_,
                                Iterator end_,
                                std::size_t size_) {
    m_logger->stream() << ELPP_LITERAL("[");
    for (std::size_t i = 0;
...
class Writer : base::NoCopy {
 public:
  ...
  Writer& construct(size_t count, const char* loggerIds, ...);
...
#if ELPP_VERBOSE_LOG
template <typename T, typename... Args>
inline void Logger::verbose(int vlevel,
                            const char* s,
                            const T& value,
                            const Args&... args) {
  acquireLock();  // released in Writer!
...
class SysLogInitializer {
 public:
  SysLogInitializer(const char* processIdent,
                    int options = 0,
                    int facility = 0) {
#if defined(ELPP_SYSLOG)
...
class Loggers : base::StaticClass {
 public:
  ...
  ///         successfully, it only means that it has attempeted to configure
