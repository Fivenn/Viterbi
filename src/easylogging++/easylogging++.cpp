...
Configuration::Configuration(Level level,
                             ConfigurationType configurationType,
...
bool Configurations::parseFromFile(const std::string& configurationFile,
                                   Configurations* base) {
  // We initial assertion with true because if we have assertion diabled, we
...
void Configurations::set(Level level,
                         ConfigurationType configurationType,
                         const std::string& value) {
  base::threading::ScopedLock scopedLock(lock());
...
void Configurations::unsafeSet(Level level,
                               ConfigurationType configurationType,
                               const std::string& value) {
  Configuration* conf =
      RegistryWithPred<Configuration, Configuration::Predicate>::get(
...
void File::buildStrippedFilename(const char* filename,
                                 char buff[],
                                 std::size_t limit) {
...
void File::buildBaseFilename(const std::string& fullPath,
                             char buff[],
                             std::size_t limit,
                             const char* separator) {
...
std::string& Str::replaceAll(std::string& str,
                             char replaceWhat,
                             char replaceWith) {
...
char* Str::convertAndAddToBuff(std::size_t n,
                               int len,
                               char* buf,
                               const char* bufLim,
                               bool zeroPadded) {
...
base::type::string_t DateTime::formatTime(unsigned long long time,
  ...
  for (base::type::EnumType i = start; i < base::consts::kTimeFormatsCount - 1;
    ...
    if (base::consts::kTimeFormats[i].value == 1000.0f &&
        static_cast<float>(time) / 1000.0f < 1.9f) {
      break;
    }
      ...
      switch (*++format) {
        ...
        case 'd':  // Day
          buf = base::utils::Str::convertAndAddToBuff(
              static_cast<size_t>(tInfo->tm_mday), 2, buf, bufLim);
          continue;
        ...
        case 'M':  // month
          buf = base::utils::Str::convertAndAddToBuff(
              static_cast<size_t>(tInfo->tm_mon) + 1, 2, buf, bufLim);
          continue;
        case 'b':  // month (short)
          buf = base::utils::Str::addToBuff(
        ...
        case 'y':  // year (two digits)
          buf = base::utils::Str::convertAndAddToBuff(
              static_cast<size_t>(tInfo->tm_year + base::consts::kYearBase), 2,
              buf, bufLim);
          continue;
        case 'Y':  // year (four digits)
          buf = base::utils::Str::convertAndAddToBuff(
              static_cast<size_t>(tInfo->tm_year + base::consts::kYearBase), 4,
              buf, bufLim);
          continue;
        case 'h':  // hour (12-hour)
        ...
        case 'H':  // hour (24-hour)
          buf = base::utils::Str::convertAndAddToBuff(
              static_cast<size_t>(tInfo->tm_hour), 2, buf, bufLim);
          continue;
        case 'm':  // minute
          buf = base::utils::Str::convertAndAddToBuff(
              static_cast<size_t>(tInfo->tm_min), 2, buf, bufLim);
          continue;
        case 's':  // second
          buf = base::utils::Str::convertAndAddToBuff(
              static_cast<size_t>(tInfo->tm_sec), 2, buf, bufLim);
          continue;
...
void TypedConfigurations::build(Configurations* configurations) {
  ...
  for (Configurations::const_iterator it = configurations->begin();
    ...
    if (conf->configurationType() == ConfigurationType::Enabled) {
      ...
      // sensative operation, we can live with another loop; (by the way this
...
void TypedConfigurations::insertFile(Level level,
  ...
  std::string filePath = base::utils::File::extractPathFromFilename(
      resolvedFilename, base::consts::kFilePathSeperator);
...
Writer& Writer::construct(size_t count, const char* loggerIds, ...) {
...
void Writer::initializeLogger(const std::string& loggerId,
                              bool lookup,
                              bool needLock) {
  if (lookup) {
...
PerformanceTracker::PerformanceTracker(const std::string& blockName,
  ...
  // configuraton at time of construction
...
void PerformanceTracker::checkpoint(const std::string& id,
                                    const char* file,
...
void Helpers::crashAbort(int sig,
...
void Helpers::logCrashReason(int sig,
                             bool stackTraceIfAvailable,
                             Level level,
                             const char* logger) {
